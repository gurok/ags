...
int CreateGraphicOverlay(int xx,int yy,int slott,int trans) {
    ...
    Bitmap *screeno=BitmapHelper::CreateTransparentBitmap(spritewidth[slott],spriteheight[slott], GameResolution.ColorDepth);
...
int CreateTextOverlayCore(int xx, int yy, int wii, int fontid, int clr, const char *tex, int allowShrink) {
    if (wii<8) wii=GameSize.Width/2;
    if (xx<0) xx=GameSize.Width/2-wii/2;
    if (clr==0) clr=16;
    int blcode = crovr_id;
    crovr_id = 2;
