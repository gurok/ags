...
void get_overlay_position(int overlayidx, int *x, int *y) {
    ...
    if (screenover[overlayidx].x == OVR_AUTOPLACE) {
        ...
        if ((tdxp + screenover[overlayidx].pic->GetWidth()) >= GameSize.Width)
            tdxp = (GameSize.Width - screenover[overlayidx].pic->GetWidth()) - 1;
        if (game.chars[charid].room != displayed_room) {
            tdxp = GameSize.Width/2 - screenover[overlayidx].pic->GetWidth()/2;
            tdyp = GameSize.Height/2 - screenover[overlayidx].pic->GetHeight()/2;
        }
    }
    else {
