...
bool init_gfx_mode(const Size &game_size, const Size &screen_size, int cdep)
{
    ...
    const bool result = gfxDriver->Init(game_size.Width, game_size.Height, screen_size.Width, screen_size.Height, final_col_dep, usetup.windowed > 0, &timerloop, usetup.vsync);
...
void display_gfx_mode_error(const Size &game_size, const Size &screen_size)
{
    ...
    platform->DisplayAlert("There was a problem initializing graphics mode %d x %d (%d-bit) with game size %d x %d and filter '%s'.\n"
        ...
        "%s",
        screen_size.Width, screen_size.Height, firstDepth, game_size.Width, game_size.Height, filter ? filter->GetFilterID() : "Undefined", get_allegro_error(), platform->GetGraphicsTroubleshootingText());
}
